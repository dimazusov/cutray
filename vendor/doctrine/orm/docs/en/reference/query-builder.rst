...
A ``Command`` provides an API that is designed for
...
Constructing a new Command object
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The same way you build a normal Query, you build a ``Command``
object. Here is an example of how to build a ``Command``
object:
...
.. code-block:: php
    ...
    // example1: creating a Command instance
    $qb = $em->createQueryBuilder();

An instance of Command has several informative methods.  One
...
.. code-block:: php
    ...
    <?php
    // $qb instanceof Command

    // example2: retrieving type of Command
    echo $qb->getType(); // Prints: 0

There're currently 3 possible return values for ``getType()``:


-  ``Command::SELECT``, which returns value 0
-  ``Command::DELETE``, returning value 1
-  ``Command::UPDATE``, which returns value 2

It is possible to retrieve the associated ``EntityManager`` of the
current ``Command``, its DQL and also a ``Query`` object when
...
.. code-block:: php
    ...
    // example4: retrieve the DQL string of what was defined in Command
    $dql = $qb->getDql();

    // example5: retrieve the associated Query object with the processed DQL
    $q = $qb->getQuery();

Internally, ``Command`` works with a DQL cache to increase
performance. Any changes that may affect the generated DQL actually
modifies the state of ``Command`` to a stage we call
STATE\_DIRTY. One ``Command`` can be in two different states:


-  ``Command::STATE_CLEAN``, which means DQL haven't been
   altered since last retrieval or nothing were added since its
   instantiation
-  ``Command::STATE_DIRTY``, means DQL query must (and will)
...
``Command`` helper methods are considered the standard way to
...
Here is a complete list of helper methods available in ``Command``:
...
The Command is a builder object only -  it has no means of actually
executing the Query. Additionally a set of parameters such as query hints
cannot be set on the Command itself. This is why you always have to convert
...
You can also add a :ref:`filtering-collections` to a Command by
using ``addCriteria``:
...
All helper methods in ``Command`` actually rely on a single
...
.. code-block:: php
   ...
   // "SELECT u FROM User u WHERE u.id = ? ORDER BY u.name ASC"
   // using Command using Expr\* instances
   $qb->add('select', new Expr\Select(array('u')))
      ->add('from', new Expr\From('User', 'u'))
